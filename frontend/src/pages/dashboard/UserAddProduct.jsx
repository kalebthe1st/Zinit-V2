...
const UserAddProduct = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${backendUrl}/api/product/user/add`, formData, { headers: { token } });
            if (response.data.success) {
                ...
                setSizes([]); setPurchaseOptions([]); setBestseller(false);
            } else {
    ...
    return (
        <div>
            ...
            <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-5 p-4 box-border border rounded-lg">
                ...
                <button type="submit" className="w-32 py-3 mt-4 bg-black text-white rounded">ADD PRODUCT</button>
            </form>
        </div>
    );
};
